    ...
    public static void initialDeck() {
        ...
        for (int i = 0; i < colorValue.length-1; i++) { // 4 Repetitions ColorValue 0= R, 1 = G, 2 = B, 3 = Y
            ...
            for (int j = 1; j < faceValue.length-4; j++) {   // 11 repetitions
                cardDeck.add( new Cards(faceValue[j], colorValue[i], points[j] )); // this adds the following cards: (1copy R1 - <->, G0 - <->, B0 - <->, Y0 - <->)
                cardDeck.add( new Cards(faceValue[j], colorValue[i], points[j] )); // this adds the following cards: (2copy R1 - <->, G0 - <->, B0 - <->, Y0 - <->)
            }

            for (int j = faceValue.length-4; j < faceValue.length-2; j++) { //2 repetitions
                cardDeck.add( new Cards(faceValue[j], colorValue[i], points[points.length-2] )); //this adds the following cards: (1copy +2, +4)
                cardDeck.add( new Cards(faceValue[j], colorValue[i], points[points.length-2] )); //this adds the following cards: (2copy +2, +4)
            }
        }
